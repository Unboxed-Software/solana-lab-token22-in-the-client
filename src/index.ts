...
async function main() {
	 ...
	 */
	const regularMint = await createAndMintToken(
		CLUSTER,
		connection,
		TOKEN_PROGRAM_ID,
		keyPair,
	 ...
	 * Using TOKEN_2022_PROGRAM_ID, create a mint, create a associated token account and mint 100 tokens to that token account
	 */
	const regularMintTokenProgram = await fetchTokenProgramFromAccount(
		connection,
		regularMint
	)
	const token22MintTokenProgram = await fetchTokenProgramFromAccount(
		connection,
		token22Mint
	)

	if (!regularMintTokenProgram.equals(TOKEN_PROGRAM_ID))
		throw new Error('Regular mint token program is not correct')
	if (!token22MintTokenProgram.equals(TOKEN_2022_PROGRAM_ID))
		throw new Error('Token22 mint token program is not correct')
